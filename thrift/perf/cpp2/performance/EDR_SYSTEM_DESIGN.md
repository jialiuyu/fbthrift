...
6. 让 agent 从任一仓库进入时都能自动发现维护协议。
7. 为人工接管提供完整指导书，并用机械校验约束 agent 输出。
...
├── EDR_SYSTEM_IMPLEMENTATION_PLAN.md
├── AGENT_PROTOCOL.md
├── MAINTENANCE_GUIDE.zh.md
├── validate_edr.py
├── edr/
│   └── EDR-NNNN-<slug>.md
├── templates/
│   └── EDR_TEMPLATE.md
└── tests/
    └── test_validate_edr.py
```

`folly` 不保存镜像文档。EDR 通过 commit、branch 和文件路径引用对应的 `folly`
状态。`folly` 和 `fbthrift` 根目录各保存短小的 `AGENTS.md` / `CLAUDE.md`
路由，指向同一份 `AGENT_PROTOCOL.md`。外层 workspace 的本地指令只是当前机器的
额外入口，不是系统唯一入口，不保存实验事实，也不作为 Git 仓库维护。

## 5. 文档职责

### 5.1 `README.md`

说明目录结构、状态枚举和快速入口，分别链接 agent 协议与人工维护指导书，不复制
二者全文。
...
### 5.5 `AGENT_PROTOCOL.md`

使用短句和强制性规则定义 agent 的开工、检索、建档、状态转换、收尾和验证流程。
仓库根路由必须要求相关任务先读此文件。

### 5.6 `MAINTENANCE_GUIDE.zh.md`

面向人说明如何创建、关闭、重开、替代和修复 EDR/Frontier，并提供 agent 未遵守协议
时的人工补救步骤和 review checklist。

### 5.7 `validate_edr.py`

使用 Python 3 标准库检查 frontmatter、ID、状态、Frontier 索引、EDR 关系、相对链接、
双仓库 commit 和根目录 agent 路由。校验失败时不得提交实验记录。

## 6. EDR 数据模型

每份 EDR 使用 Markdown 正文和无嵌套的 YAML frontmatter 子集。扁平 schema 便于
agent 生成，也允许校验器在不依赖 PyYAML 的环境中运行。frontmatter 至少包含：
...
两个仓库根目录的 `AGENTS.md` / `CLAUDE.md` 负责自动发现，规范内容只在
`AGENT_PROTOCOL.md` 中维护。协议对跨仓库性能任务规定：
...
6. 提交前必须运行 `validate_edr.py`；校验失败时不得宣称维护闭环完成。
...
6. 创建两仓库根路由、agent 协议、人的维护指导书和无第三方依赖的校验器。
...
- 两仓库根目录的 `AGENTS.md` / `CLAUDE.md` 都能路由到 agent 协议。
- 人工指导书覆盖 agent 不遵从协议时的修复流程。
- 校验器自动测试和真实 paired-worktree 校验均通过。
