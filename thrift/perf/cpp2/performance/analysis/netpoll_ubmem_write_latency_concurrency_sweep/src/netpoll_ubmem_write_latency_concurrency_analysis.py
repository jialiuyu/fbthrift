...
MARKERS = {1: "o", 4: "s", 16: "^", 64: "D", 256: "P", 512: "X"}
        ...
        for raw in csv.DictReader(source):
            rows.append(
                Measurement(
                    ...
                    write_cost_ns=int(raw["write_cost_ns"]),
...
def derive_measurements(
    ...
    by_cell = {(row.concurrency, row.write_cost_ns): row for row in rows}
    ...
    for row in sorted(
        rows, key=lambda item: (item.write_cost_ns, item.concurrency)
    ):
        ...
        derived.append(
            DerivedMeasurement(
                ...
                write_cost_ns=row.write_cost_ns,
                cost_multiplier=row.write_cost_ns / REFERENCE_NS,
                tps=row.tps,
                tp99_us=row.tp99_us,
...
def _series_for_concurrency(
    ...
    return [
        next(
            row
            for row in rows
            if row.concurrency == concurrency and row.write_cost_ns == cost
        )
...
def _plot_sensitivity(
    ...
    for concurrency in CONCURRENCIES:
        series = _series_for_concurrency(rows, concurrency)
        style = {
    ...
    axes[0].axvspan(0, 4, color="#DDEBF7", alpha=0.22, zorder=0)
    axes[1].axvspan(0, 4, color="#DDEBF7", alpha=0.22, zorder=0)
...
def _plot_scaling(
    ...
    axes[1].set_yticks((50, 100, 200, 500, 1000, 2000, 5000))
    ...
    for axis in axes:
        ...
        axis.grid(True, axis="y", which="major")
    axes[0].legend(
        loc="lower right",
        frameon=True,
        framealpha=0.94,
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=project_dir
        / "data"
        / "netpoll_ubmem_write_latency_concurrency_derived_20260810.csv",
    )
    parser.add_argument(
